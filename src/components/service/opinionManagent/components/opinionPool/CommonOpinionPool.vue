<template>
  ...
  <div v-if="act.show" ref="dragBox" v-drag class="common_opinion_pool_box" @click="updateDragBoxZindex">
    <div class="title">
      <span>{{ title }}</span>
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      if (this.rewriteInterfaces?.getCommonOpinionsFunc) {
        this.rewriteInterfaces.getCommonOpinionsFunc(params).then(res => {
        ...
        api.getCommonOpinions(params, this.apiHost).then(res => {
          if (res.data.code === 'SUCCESS') {
            _this.act.list = res.data.detail.records || []
            _this.act.pageInfo.total = res.data.detail.total
          } else {
